...
const Sidebar = () => {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-screen bg-surface border-r border-[#334155] transition-all duration-300 z-50 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'}`}>
      <div className="flex items-center justify-between p-4 border-b border-[#334155]">
        <div className={`flex items-center space-x-3 overflow-hidden ${!sidebarOpen && 'hidden'}`}>
          ...
          <span className="text-xl font-bold font-sans tracking-tight">AthletOS</span>
        </div>
        <button 
              ...
              className={({ isActive }) => 
                `flex items-center p-3 rounded-xl transition-all duration-200 group ${
                  isActive 
                    ...
                    : 'text-gray-400 hover:bg-[#334155]/50 hover:text-gray-100'
                }`
              }
      ...
      <div className="p-4 border-t border-[#334155]">
        <div className={`flex items-center ${sidebarOpen ? 'space-x-3' : 'justify-center'}`}>
          <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-accent to-purple-500 flex items-center justify-center flex-shrink-0">
            <span className="text-white font-bold text-sm">US</span>
          </div>
          {sidebarOpen && (
            <div className="overflow-hidden">
              <p className="text-sm font-medium text-gray-200 truncate">Athlete</p>
              <div className="flex items-center space-x-1">
                <span className="w-2 h-2 rounded-full bg-success"></span>
                <span className="text-xs text-gray-400 truncate">Optimal Load</span>
              </div>
            </div>
          )}
        </div>
