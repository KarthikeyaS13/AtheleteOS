  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Today's Snapshot</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-400">Training Load</p>
            <div className="flex items-baseline gap-2">
              <h2 className="text-3xl font-bold text-white">{Math.round(todaysLoad)}</h2>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div className="w-full">
            <p className="text-sm font-medium text-gray-400">Weekly Volume (km)</p>
            <div className="flex items-baseline justify-between">
              <h2 className="text-3xl font-bold text-white">{Math.round(weeklyDistance)}</h2>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-400">Nutrition (kcal)</p>
            <div className="flex items-baseline justify-between">
              <h2 className="text-3xl font-bold text-white">{totalCals}</h2>
            </div>
            <p className="text-xs text-gray-500 mt-1">Carbs 140g | Prot 80g | Fat 40g</p>
          </div>
        </Card>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div className="w-full">
            <p className="text-sm font-medium text-gray-400">Recovery Score</p>
            <h2 className="text-3xl font-bold text-white">{recoveryScore.toFixed(1)} <span className="text-base text-gray-500 font-normal">/ 10</span></h2>
            <div className="w-full bg-surface border border-[#334155] rounded-full h-1.5 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="col-span-1">
          <h3 className="text-lg font-bold text-white mb-6">7-Day Training Load</h3>
        ...
        <Card className="col-span-1 flex flex-col justify-between">
            <h3 className="text-lg font-bold text-white mb-2">Upcoming Races</h3>
            <div className="space-y-3 mt-4 flex-1">
                {races.slice(0,3).map(race => {
                   const daysTo = differenceInDays(new Date(race.date), new Date());
                   return (
                    <div key={race.id} className="flex items-center justify-between p-3 rounded-xl bg-background border border-[#334155] border-l-4 border-l-accent hover:border-accent transition-colors">
                      <div className="flex items-center gap-3">
                         <div className="p-2 bg-accent/10 rounded-lg text-accent"><CalIcon size={20}/></div>
                         <div>
                            <p className="text-white font-medium">{race.name}</p>
                            <p className="text-xs text-gray-500">{race.distance}km • Goal: {race.targetTime}</p>
                         </div>
                      </div>
                      <div className="text-right">
                         <p className="text-2xl font-bold text-white">{Math.max(0, daysTo)}</p>
            ...
            <h3 className="text-lg font-bold text-white mb-2 mt-6">Recent PRs</h3>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                 {/* Mock display of PRs */}
                 <div className="flex-shrink-0 bg-background border border-[#334155] rounded-xl p-3 w-40">
                    <div className="flex items-center gap-2 text-warning mb-2"><Trophy size={16}/> <span>5K Run</span></div>
                    <p className="text-xl font-bold text-white">00:22:30</p>
                    <p className="text-xs text-success">was 00:23:15</p>
                 </div>
                 <div className="flex-shrink-0 bg-background border border-[#334155] rounded-xl p-3 w-40">
                    <div className="flex items-center gap-2 text-warning mb-2"><Trophy size={16}/> <span>100K Cycle</span></div>
                    <p className="text-xl font-bold text-white">03:20:00</p>
